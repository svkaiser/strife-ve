...
net_addr_t *NET_Query_ResolveMaster(net_context_t *context)
{
    ...
    // [SVE]: We don't have a master server for Veteran Edition and it would be
    // rude to make UDP clients show up on master.chocolate-doom.org w/o special
    // permission, so this has been conditionalized out for the commercial build.
#ifdef ALLOW_CHOCO_MASTER_SERVER
...
boolean NET_StartSecureDemo(prng_seed_t seed)
{
    ...
    if (response != NULL)
    {
        if (NET_ReadPRNGSeed(response, seed))
        {
            ...
            if (signature != NULL)
            {
                securedemo_start_message = M_Strdup(signature);
                result = true;
            }
        }
