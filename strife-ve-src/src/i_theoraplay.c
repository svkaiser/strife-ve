...
typedef struct nx_set_t { unsigned long __bits[8 / sizeof(long)]; } nx_set_t;
 

#define __NX_op_S(i, size, set, op) ( (i)/8U >= (size) ? 0 : \
	((set)->__bits[(i)/8/sizeof(long)] op (1UL<<((i)%(8*sizeof(long))))) )

#define NX_ZERO(set) memset(set,0,sizeof(nx_set_t))
#define NX_SET(i, set) __NX_op_S(i,sizeof(nx_set_t),set,|=)
#endif

static inline int Thread_Create(TheoraDecoder *ctx, void *(*routine) (void*))
{
	int x = pthread_create(&ctx->worker, NULL, routine, ctx);
#ifdef SVE_PLAT_SWITCH
	// dimitrisg 20200618: on NX run the thread on Core 2 
	// otherwise the video stutters like crazy
	nx_set_t core2;
	NX_ZERO(&core2);
	NX_SET(2, &core2);
	pthread_setaffinity_np(ctx->worker, sizeof(nx_set_t), &core2);
#endif
	return x;
}
static inline void Thread_Join(THEORAPLAY_THREAD_T thread)
...
static inline void Mutex_Lock(THEORAPLAY_MUTEX_T mutex)
{
	...
	// dimitrisg 20200618: this crashes under NX , i dont know why...
...
void I_AVStartVideoStream(const char *fname) 
{
	...
	initfailed = quit = (!videoBuffer || !windowscreen);
	...
	while (!quit && THEORAPLAY_isDecoding(decoder)) {
		...
		while (windowscreen && SDL_PollEvent(&event)) {
			switch (event.type) {
