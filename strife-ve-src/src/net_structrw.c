...
void NET_WriteSettings(net_packet_t *packet, net_gamesettings_t *settings)
{
    ...
    NET_WriteInt8(packet, settings->classicmode); // [SVE]
    NET_WriteInt8(packet, settings->maxgore);     // [SVE]
...
boolean NET_ReadSettings(net_packet_t *packet, net_gamesettings_t *settings)
{
    ...
    success = NET_ReadInt8(packet, (unsigned int *) &settings->ticdup)
           ...
           && NET_ReadInt8(packet, (unsigned int *) &settings->classicmode) // [SVE]
           && NET_ReadInt8(packet, (unsigned int *) &settings->maxgore)     // [SVE]
                         ...
                         boolean lowres_turn)
{
    ...
    NET_WriteInt16(packet, diff->diff); // [SVE]: need larger range
    ...
    if (diff->diff & NET_TICDIFF_PITCH)
        NET_WriteInt16(packet, diff->cmd.pitchmove);
    ...
    if (diff->diff & NET_TICDIFF_STRIFE1)
        NET_WriteInt8(packet, diff->cmd.buttons2);
    if (diff->diff & NET_TICDIFF_STRIFE2)
        NET_WriteInt16(packet, diff->cmd.inventory); // [SVE]: must be 16-bit
}
                           ...
                           boolean lowres_turn)
{
    ...
    if (!NET_ReadInt16(packet, &diff->diff)) // [SVE]: larger diff
        return false;
    ...
    if (diff->diff & NET_TICDIFF_STRIFE2)
    {
        if (!NET_ReadInt16(packet, &val)) // [SVE]: must be 16-bit
            return false;
...
void NET_TiccmdDiff(ticcmd_t *tic1, ticcmd_t *tic2, net_ticdiff_t *diff)
{
    ...
    // [SVE] svillarreal
    if (tic1->pitchmove != tic2->pitchmove)
    ...
    if (tic1->buttons2 != tic2->buttons2)
        diff->diff |= NET_TICDIFF_STRIFE1;

    if (tic2->inventory != 0)
        diff->diff |= NET_TICDIFF_STRIFE2;
}
...
void NET_TiccmdPatch(ticcmd_t *src, net_ticdiff_t *diff, ticcmd_t *dest)
{
    ...
    // [SVE] svillarreal
    if (diff->diff & NET_TICDIFF_PITCH)
        dest->pitchmove = diff->cmd.pitchmove;
